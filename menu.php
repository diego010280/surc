...
// Modulo vif
$movif = mysqli_query($conexion, "SELECT modulo_id FROM segusuario.modulos WHERE modulo_descrip = 'VIF'")  //web   //local VIF
	or die('Problemas con mod consulta surc');
$row_movif = mysqli_fetch_array($movif);
...
$obj = mysqli_query ($conexion, "SELECT DISTINCT(objetos.objeto_id) AS IdObjeto, objetos.objeto_descrip,
	permiso_ejec
	FROM autorizacion
	INNER JOIN perfiles ON perfiles.perfil_id = autorizacion.autorizacion_perfil_id
    INNER JOIN objetos ON objetos.objeto_modulo_id = perfiles.perfil_modulo_id
	INNER JOIN permisos ON permisos.permiso_perfil_id = perfiles.perfil_id AND permisos.permiso_objeto_id = objetos.objeto_id
	WHERE autorizacion.autorizacion_usuario_id = '$usid' AND perfiles.perfil_modulo_id = '$mod' 
	AND objetos.objeto_activo = 1 AND objetos.objeto_titulo = 1
	AND (objetos.objeto_padre IS NULL or objetos.objeto_padre=0) AND objetos.objeto_vercomo IN (1,3)") or die("Problemas con select obj: ".mysqli_error($conexion));;
    ...
    if ($num_row_obj > 0) {
		...
		if ($num_row_perfil >0) {
				...
				while ($row_perfil = mysqli_fetch_array($perfil)) {
								if ($row_perfil['perfil_id']==97) { /*web 154 local 97*/
									
									$autorizacion=mysqli_query($conexion,"SELECT * FROM segusuario.autorizacion
																	where autorizacion_perfil_id='$row_perfil[perfil_id]' and autorizacion_usuario_id='$usid';")
																	or die("Problemas con perfil autorizacion vif: ".mysqli_error($conexion));
									$num_row_autorizacion = $autorizacion->num_rows;
									
									if ($num_row_autorizacion>0){?>
										<li>
										...
										<input type="hidden" name="moduloid" value="<?=$row_movif['modulo_id']?>">
										</form>
