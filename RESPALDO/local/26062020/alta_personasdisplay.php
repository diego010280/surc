...
if (!empty($_REQUEST['btn_guardar_persona_upd']) and $_REQUEST['btn_guardar_persona_upd']=='Insertar') {
  ...
  $SURC_Persona_Id=$id_persona+1;
    ...
    $SURC_Persona_Apellido=(empty($_REQUEST['SURC_Persona_Apellido'])) ? '' : strtoupper(utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_Apellido'])));
    $SURC_Persona_Nombre= (empty($_REQUEST['SURC_Persona_Nombre'])) ? '' : strtoupper(utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_Nombre'])));
    $SURC_Persona_Sexo= strtoupper(utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_Sexo'])));
    ...
    $SURC_Persona_Documento=utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_Documento']));
    $SURC_Persona_ApellidoNombre=$SURC_Persona_Apellido.', '.$SURC_Persona_Nombre;
    $SURC_Persona_Alias=strtoupper(utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_Alias'])));
    $SURC_Persona_Edad= intval($_REQUEST['SURC_Persona_Edad']);
    $SURC_Persona_IdApeNom=$SURC_Persona_Id.' - '.$SURC_Persona_ApellidoNombre;
    $SURC_Persona_Telefono=utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_Telefono']));
    ...
    $SURC_Persona_ApellidoPadre=strtoupper(utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_ApellidoPadre'])));
    $SURC_Persona_NombrePadre=strtoupper(utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_NombrePadre'])));
    $SURC_Persona_ApellidoMadre=strtoupper(utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_ApellidoMadre'])));
    $SURC_Persona_NombreMadre=strtoupper(utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_Persona_NombreMadre'])));

    mysqli_query($conex_surc,"SET NAMES 'utf8'");
    mysqli_query ($conex_surc, "INSERT INTO surc_persona(
...
VALUES ('$SURC_Persona_Id','$SURC_Persona_Apellido','$SURC_Persona_Nombre',NULLIF('$SURC_Persona_Sexo',''), NULLIF('$SURC_Persona_FechNacim',''),
        ...
        or die("Problemas con el insert datos surc_personas : ".mysqli_error($conex_surc));;
 ...
 <body>
  ...
  <section>
    <article class="artwo">
      ...
      <ul class="tabs group">
        <!-- <li class="active"> <a href="#">Datos de la Persona</a></li> -->
        <li <?php if ($activo==0) echo 'class="active"' ?>> <a href="alta_personas.php?tab=0">Datos Pers.</a></li>
        <li <?php if ($activo==1) echo 'class="active"' ?> ><a href="alta_personas.php?tab=1">Domicilio Pers.</a> </li>
      </ul>


      <div id="content">
        <form class="" action="alta_personas.php" method="get">
    ...
    <form class="" action="surc_personas_ins.php" id="salir" method="get">
