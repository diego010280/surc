    ...
    if (empty($_REQUEST['SURC_TipoElemento_Id']) and empty($_REQUEST['SURC_FormaElemento_Id']) AND
      ...
      //mostrar mensaje agregar un valor
        ...
        $elementos=mysqli_query($conex_surc,"SELECT
                ...
                surc.surc_sumarioelemento.SURC_SumarioElemento_Obs AS 'Observaciones', surc.surc_formaelemento.SURC_FormaElemento_Descrip AS 'Condicion',
                surc.surc_sumarioelemento.SURC_SumarioElemento_Estado as 'estado'
                 FROM surc.surc_sumarioelemento
                ...
                ORDER BY surc.surc_sumario.SURC_Sumario_Anio DESC,surc.surc_sumario.SURC_Sumario_FechaDel DESC
                ...
                <tbody>
                    <tr>
                      ...
                      <td>
                        ...
                        <td>
                          <div>
                                <button type="submit" class="btncel" name="excel_elem" value="excel_elem" formaction="excel_elemento.php" name="button"><span class="soloimagen">Excel</span></button>
                          </div>
      ...
      <section id='grilla'>
        <table id="tabla_abm">
              <thead>
                <tr>
                  <th width="3%">N°</th>
                  <th width="5%">Nro. Sum.</th>
                  <th width="5%">Año Sum.</th>
                  <th width="20%">Dependencia</th>
                  <th width="5%">Fecha Del.</th>
                  <th width="5%">Forma</th>
                  <th width="5%">Elemento</th>
                  <th width="10%">Nro. de Serie</th>
                ...
                <?php
                  ...
                  if (!empty($num_row_elementos)) {
                      ...
                      ++$impresos;?>
                        <tr>
                          <td align="right" <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= $impresos ?></td>
                          <td align="right" <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= utf8_encode($row_elementos['Nro_sum_mp']) ?></td>
                          <td align="center" <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= utf8_encode($row_elementos['Anio_sum']) ?></td>
                          <td <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= utf8_encode($row_elementos['desc_dependencia']) ?></td>
                          <td align="center" <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= date("d-m-Y",strtotime($row_elementos['fecha_Delito'])) ?></td>
                          <td align="center" <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= utf8_encode($row_elementos['forma']) ?></td>
                          <td <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= utf8_encode($row_elementos['Tipo_Elemento']) ?></td>
                          <td <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= utf8_encode($row_elementos['Numero_de_Serie']) ?></td>
                          <td align="center" <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= $row_elementos['Cantidad'] ?></td>
                          <td <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?= utf8_encode($row_elementos['Observaciones']) ?></td>
                          <td <?php
                          if ($row_elementos['estado']==1) {
                            echo "class='mdl-color-verde'";
                          } ?>><?php
