...
if (isset($_REQUEST['buscador']) or isset($_REQUEST['go_back'])) {
    if (empty($_REQUEST['palabra_clave_pers']) and empty($_REQUEST['desdehecho']) and empty($_REQUEST['hastahecho'])) {
      //mostrar mensaje agregar un valor
        ...
        $personas=mysqli_query($conex_surc,"SELECT * FROM surc.personasumario_vif
        								where (concat_ws(' ',Documento,ApellidoyNombre,Alias,Telefono) LIKE '%$palabra_clave_pers%' or '$palabra_clave_pers'='')
                        AND (surc.personasumario_vif.Fecha_delito>='$desdehecho' or '$desdehecho'='')
                        AND (surc.personasumario_vif.Fecha_delito<= '$hastahecho' or '$hastahecho'='')
        ...
        if (!isset($_REQUEST['go_back'])) {
                    $total=mysqli_query($conex_surc,"SELECT count(*) as totalregistros  FROM surc.personasumario_vif
                    								where (concat_ws(' ',Documento,ApellidoyNombre,Alias,Telefono) LIKE '%$palabra_clave_pers%' or '$palabra_clave_pers'='')
                                    AND (surc.personasumario_vif.Fecha_delito>='$desdehecho' or '$desdehecho'='')
                                    AND (surc.personasumario_vif.Fecha_delito<= '$hastahecho' or '$hastahecho'='')
                                    LIMIT $inicio, $tamano_pagina") or die("Problemas con la consulta personas : ".mysqli_error($conex_surc));
  ...
  <section>
    <article id="artUno">
      ...
      <section id="buscador_elemen">
        <h1>Personas de Sumario VIF</h1>
        <br>

        <div id="find">
          <div id="cpanel1">
            <form  action="consulta_personas_vif.php" method="post" autocomplete="off">
            ...
            <form action="consulta_personas_vif.php" method="post" id="limpiar"></form>

          <form action="excel_pers_cons.php" method="post" id="excel"></form>
                  ...
                  if (!empty($num_row_personas)) {
                      ...
                      ++$impresos;?>
                        <tr>
                          ...
                          <td>
                            <form action="sum_d_personas_vif.php" method="post">
