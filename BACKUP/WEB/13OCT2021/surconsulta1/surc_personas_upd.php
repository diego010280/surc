...
if (isset($_REQUEST['btn_guardarpersona_upd']) and $_REQUEST['btn_guardarpersona_upd']=='Guardar Cambios') {
          ...
          $SURC_PersonaSumario_Obs=(empty($_REQUEST['SURC_PersonaSumario_Obs'])) ? '' : mb_strtoupper(utf8_encode(utf8_decode(mysqli_real_escape_string($conex_surc,$_REQUEST['SURC_PersonaSumario_Obs']))));
          ...
          $row_usuario=mysqli_fetch_array($usuario);
            ...
            $surc_sumariopersona_auditoria=mysqli_query($conex_surc,"SELECT * FROM surc.surc_sumariopersona
                            where (surc.surc_sumariopersona.SURC_Sumario_Id='$_SESSION[SURC_Sumario_Id]') and (surc.surc_sumariopersona.SURC_PersonaSumario_IdPersona='$_SESSION[SURC_PersonaSumario_IdPersona]');")
                            or die("Problemas con el select auditoria_surc_sumariopersona: ".mysqli_error($conex_surc));

            mysqli_query($conex_surc, "UPDATE surc.surc_sumariopersona SET

               SURC_PersonaSumario_IdTipoPers=NULLIF('$SURC_PersonaSumario_IdTipoPers',''),
               SURC_PersonaSumario_IdClasePer=NULLIF('$SURC_PersonaSumario_IdClasePer',''),
               SURC_PersonaSumario_IdVinculo=NULLIF('$SURC_PersonaSumario_IdVinculo',''),
               ...
               if (!empty(mysqli_affected_rows($conex_surc))) {
                 ...
                 // insertar registro
                  ...
                  while ($row_sumariopersona_auditoria=mysqli_fetch_array($surc_sumariopersona_auditoria)) {


                    if ($row_sumariopersona_auditoria['SURC_PersonaSumario_IdTipoPers']!=$SURC_PersonaSumario_IdTipoPers){
                         $activ=$activ.' '."SURC_PersonaSumario_IdTipoPers".':'.$row_sumariopersona_auditoria['SURC_PersonaSumario_IdTipoPers'].' '.$SURC_PersonaSumario_IdTipoPers;
                     }

                    if ($row_sumariopersona_auditoria['SURC_PersonaSumario_IdClasePer']!=$SURC_PersonaSumario_IdClasePer) {
                         $activ=$activ.' '."SURC_PersonaSumario_IdClasePer".':'.$row_sumariopersona_auditoria['SURC_PersonaSumario_IdClasePer'].' '.$SURC_PersonaSumario_IdClasePer;
                    }

                    if ($row_sumariopersona_auditoria['SURC_PersonaSumario_IdVinculo']!=$SURC_PersonaSumario_IdVinculo) {
                        $activ=$activ.' '."SURC_PersonaSumario_IdVinculo".':'.$row_sumariopersona_auditoria['SURC_PersonaSumario_IdVinculo'].' '.$SURC_PersonaSumario_IdVinculo;
                    }

                    if ($row_sumariopersona_auditoria['SURC_PersonaSumario_EsMenor']!=$SURC_PersonaSumario_EsMenor) {
                          $activ=$activ.' '."SURC_PersonaSumario_EsMenor".':'.$row_sumariopersona_auditoria['SURC_PersonaSumario_EsMenor'].' '.$SURC_PersonaSumario_EsMenor;
                    }

                    if ($row_sumariopersona_auditoria['SURC_PersonaSumario_Obs']!=$SURC_PersonaSumario_Obs) {
                            $activ=$activ.' '."SURC_PersonaSumario_Obs".':'.$row_sumariopersona_auditoria['SURC_PersonaSumario_Obs'].' '.$SURC_PersonaSumario_Obs;
                    }
                  }
          ...
          <table class="plus conmargen">
            ...
            <tr>
              ...
              <tr>
                <td>Observaciones</td>
                <td>
                  <textarea name="SURC_PersonaSumario_Obs" rows="8" cols="80" style="overflow-y:scroll;text-align:justify;text-transform:uppercase;"><?php if (!empty($row_personasum['SURC_PersonaSumario_Obs'])) {
                    echo utf8_encode($row_personasum['SURC_PersonaSumario_Obs']);
                  }else {
                    echo "";
                  } ?></textarea>
            ...
            <table>
              <tr>
                ...
                <td>
                  <button type="submit" name="btn_upd_persona" value="mod_persona" class="btnguardar" form="salir">Modificar Datos Pers.</button>
                </td>
              </table>
      </form>

      <form action="alta_personas_upd.php" id="salir" method="post">
          <input type="hidden" name="persona_id" value="<?php echo $_SESSION['SURC_PersonaSumario_IdPersona']; ?>">
      </form>
